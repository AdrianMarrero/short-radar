...
from app.scoring.probabilistic import (
    compute_pead_drift,
    simulate_signal_probabilities,
)
...
def _horizon_to_days(horizon: str) -> int:
    """Map legacy horizon string to a numeric day count.

    Mirrors ``jobs.daily._horizon_days_for`` so the Monte Carlo sim
    uses the same horizon the signal tracker will use to expire signals.
    """
    h = (horizon or "").lower()
    if h == "intraday":
        return 5
...
def _compute_position_size(
    ...
    """Position sizing per signal — fixed fractional risk model.

    risk_per_trade_eur = capital_eur * risk_pct  (e.g. 1% of 2000€ = 20€)
    risk_per_share     = entry - stop            (in instrument currency, EUR/USD ~= 1)
    shares             = floor(risk_per_trade_eur / risk_per_share)
    position_size_eur  = shares * entry

    Returns (None, None) when inputs are missing or risk is non-positive.
    """
    import math as _math
    if entry is None or stop is None:
        return None, None
    if not (_math.isfinite(entry) and _math.isfinite(stop)):
        return None, None
    risk_per_share = entry - stop
    if risk_per_share <= 0:
        return 0, 0.0
    risk_per_trade_eur = float(capital_eur) * float(risk_pct)
    if risk_per_trade_eur <= 0:
        return 0, 0.0
    shares = int(risk_per_trade_eur // risk_per_share)
    if shares < 0:
        shares = 0
    position_size_eur = round(shares * float(entry), 2)
    return shares, position_size_eur
...
def compute_final_score(
    ...
    *,
    ...
    # ---- Position sizing (per-signal, fixed fractional risk) ----
    eff_capital = float(capital_eur if capital_eur is not None else settings.default_capital_eur)
    eff_risk_pct = float(risk_per_trade_pct if risk_per_trade_pct is not None else settings.risk_per_trade_pct)
    shares, position_size_eur = _compute_position_size(
        plan.entry, plan.stop, eff_capital, eff_risk_pct,
    )

    # ---- Probabilistic display layer (Monte Carlo, GBM) ----
    # Skips silently when realized vol or plan fields are missing. NOT used
    # as a gate or as a factor — pure informational layer for the UI.
    realized_vol = getattr(snap, "realized_vol_30d", None) if snap else None
    em_score = factor_scores.get("earnings_momentum")
    drift = compute_pead_drift(em_score)
    mc_result = None
    if realized_vol is not None:
        mc_result = simulate_signal_probabilities(
            entry=plan.entry,
            stop=plan.stop,
    ...
    raw_score_data = {
        ...
        "rejection_reasons": [],
        "gates": gates.to_dict(),
        # Position sizing — surfaced per-signal so the UI can show €/shares.
        "shares": shares,
        ...
        # Probabilistic display layer (Monte Carlo). None when no vol data.
        "probabilistic": _safe_dict(mc_result.to_dict()) if mc_result else None,
    }
