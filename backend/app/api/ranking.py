...
from app.services.portfolio import (
    PortfolioState,
    evaluate_signal_against_portfolio,
...
def _row_to_score_out(score: ShortScore, inst: Instrument, tech) -> ScoreOut:
    ...
    return ScoreOut(
        ...
        position_size_eur=raw.get("position_size_eur"),
        shares=raw.get("shares"),
        prob_target_1=prob.get("p_hit_target_1"),
        prob_target_2=prob.get("p_hit_target_2"),
        prob_stop=prob.get("p_hit_stop"),
        prob_expire=prob.get("p_expire"),
        expected_r=prob.get("expected_r"),
        edge_class=prob.get("edge_class"),
    )
...
def _apply_portfolio_warnings(rows: list[ScoreOut], portfolio: PortfolioState) -> list[ScoreOut]:
    """Tag each row with portfolio-aware warnings (cartera/sector/capital)."""
    for r in rows:
        extra = evaluate_signal_against_portfolio(
            instrument_id=r.instrument_id,
            sector=r.sector,
            position_size_eur=r.position_size_eur,
            portfolio=portfolio,
        )
        if extra:
            r.warnings = merge_warnings(r.warnings, extra)
    return rows
...
def get_ranking(
    ...
    portfolio = load_portfolio_state(db, get_settings().default_capital_eur)
    out = _apply_portfolio_warnings(out, portfolio)
